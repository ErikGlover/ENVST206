...
dirR <- "/Users/erikglover/Documents/Environmental Data/a08/oneida"
...
rgbSfalse <- stack(rdatB4,rdatB3,rdatB8)/10000
plotRGB(rgbSfalse, stretch="lin")
...
#different visible light bands vs infrared light 
...
Extract <- raster::extract(NDVI,landExtract[,2:3])
NDVIEx <- cbind(landExtract,Extract)


#violin plot
ggplot(NDVIEx[NDVIEx$landcID==c("agri", "forest", "wetland"),], aes(x=landcID, y=Extract))+geom_violin(fill="#FFCC00")+geom_boxplot(width=0.1,size=0.25, fill="grey90")+theme_classic()+labs(y="NDVI", x="land cover type")
